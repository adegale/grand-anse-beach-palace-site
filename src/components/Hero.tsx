...
const MEDIA = [
  ...
  { type: 'vid', src: 'clip-waves.mp4',        dur: 6.5, from: 'scale(1.04)',                       to: 'scale(1.12)' },
  ...
  { type: 'vid', src: 'clip-water.mp4',        dur: 6.5, from: 'scale(1.04)',                       to: 'scale(1.12)' },
...
export default function Hero() {
  ...
  function show(i: number) {
    ...
    if (item.type === 'vid') {
      ...
      let started = false;
      // Use canplay — fires once video can play without seeking; avoids seek-race twitch
      const go = () => { if (started) return; started = true; reveal(); };
      v.addEventListener('canplay', go);
      v.play().catch(() => {});
      setTimeout(go, 800); // fallback if canplay is slow
  ...
  return (
    <section style={{ position: 'relative', width: '100%', height: '100vh', minHeight: 560, overflow: 'hidden', background: '#0a3a44', fontFamily: "'Figtree', system-ui, sans-serif" }}>
      ...
      <div style={{ position: 'absolute', left: 0, bottom: 0, padding: '0 clamp(22px,5vw,72px) clamp(48px,7vh,86px)', zIndex: 3, maxWidth: 'min(880px,92%)' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 14, alignItems: 'center' }}>
          <a href="#contact" style={{ display: 'inline-flex', alignItems: 'center', gap: 9, textDecoration: 'none', fontWeight: 700, fontSize: 17, padding: '16px 28px', borderRadius: 999, background: '#ff6a3d', color: '#fff', boxShadow: '0 8px 26px rgba(255,106,61,0.45)' }}>
            Book your stay <span style={{ fontSize: 19 }}>&rarr;</span>
          </a>
      ...
      <div className="hero-scroll" style={{ position: 'absolute', left: '50%', bottom: 22, transform: 'translateX(-50%)', zIndex: 3, color: 'rgba(255,255,255,0.8)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5, animation: 'gabp-bob 2.4s ease-in-out infinite' }}>
        ...
        <span style={{ fontSize: 16 }}>&darr;</span>
      </div>
